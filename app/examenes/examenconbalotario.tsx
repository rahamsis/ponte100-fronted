...
const ExamenConBalotario: React.FC<QuestionnaireProps> = ({
    questions,
    selectedAnswers,
    setSelectedAnswers,
    currentQuestion,
            ...
            <div className="mx-auto md:w-full">
                <div className={`x:flex flex-row space-x-3`}>
                    <div className="hidden x:block ">
                        <div className="relative bg-white w-full rounded-lg border shadow-sm py-4">
                            <h3 className="mb-6 text-center text-lg font-medium">Tabla de Preguntas</h3>
                            <div className="grid grid-cols-8 gap-1 px-2 w-full">
                                {questions.map((_, index) => (
                                    <div
                                        key={index + 1}
                                        // onClick={() => setCurrentQuestion(index + 1)}
                                        // pinta el background si se ha seleccionado alguna alternativa
                                        className={`flex-1 flex items-center justify-center px-3 py-1 rounded-md border text-sm 
                                        transition-all ${currentQuestion === index + 1 && "border-balotario"}                                     
                                        ${selectedAnswers[index + 1] !== undefined ? "bg-button2 text-white hover:bg-green-600" : "text-balotario"}`}>
                        ...
                        <div className="mx-auto w-full rounded-lg border p-6 shadow-sm bg-white">
                            ...
                            <div className="flex flex-row space-x-3 mb-3">
                                ...
                                {questions[currentQuestion - 1].intentos !== undefined &&
                                    <div className="text-sm w-1/6 font-semibold lg:flex flex-row justify-center items-center hidden">
                                        {/* Icono con tooltip */}
                            ...
                            <div className="flex flex-row space-x-4">
                                ...
                                <div className={`${questions[currentQuestion - 1].intentos !== undefined ? 'hidden' : ''} md:flex border rounded-md p-2 items-center`}>
                                    <div className="pr-2">
                                        <Clock className={`w-10 h-10 ${timeExpired ? "text-red-500" : ""}`} />
                                    </div>
                                    <div>
...
export default ExamenConBalotario;
